...
class ResidualUnetLoss(nn.Module):
    def __init__(self, r=0.1):
        super(ResidualUnetLoss, self).__init__()
        self.r = r

    def forward(self, y1, label):
        d = y1 - label
        lossf1 = torch.abs(d).mean() - self.r * torch.abs(d.mean())
        loss = F.mse_loss(y1, label) + lossf1 
        return loss
    
class DewarpingLoss(nn.Module):
    def __init__(self, r=0.1):
        super(DewarpingLoss, self).__init__()
        self.r = r
...
class MS_SSIM_Loss(MS_SSIM):
    ...
    def forward(self, img1,img_target):
        return 100*( 1 - super(MS_SSIM_Loss, self).forward(img1, img_target) )
    
class DewarpingMS_SSIM_Loss(MS_SSIM):
    
    def forward(self, img1, img2,img_target):
#         print(img1,img2,img_target)
        return 100*( 1 - super(DewarpingMS_SSIM_Loss, self).forward(img1, img_target) )+\
            100*( 1 - super(DewarpingMS_SSIM_Loss, self).forward(img2, img_target) )